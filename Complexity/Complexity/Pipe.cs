...
namespace Complexity
{
    public class Pipe
    {
        ...
        public void updateTexture()
        {
            ...
            if (this.X > 0)
            {
                if (Grid.pipes[this.X - 1, this.Y].active)
                {
                    if (Grid.pipes[this.X - 1, this.Y].type == this.type)
                    {
                        this.left = true;
                    }
                }
            ...
            if (this.Y > 0)
            {
                if (Grid.pipes[this.X, this.Y - 1].active)
                {
                    if (Grid.pipes[this.X, this.Y - 1].type == this.type)
                    {
                        this.up = true;
                    }
                }
        ...
        {
            foreach (Pipe p in pipeList.ToArray())
            {
                if (p.active)
                {
                    Rectangle src = new Rectangle(p.junctionType*64, 0, 64, 64);
                    spriteBatch.Draw(Main.pipeTexture[p.type], new Rectangle(p.X*64 - 32 - (int) Main.camPos.X, p.Y*64 - 32 - (int) Main.camPos.Y, 64, 64), src, Color.White, p.rotation, new Vector2(32, 32), SpriteEffects.None, 0f);
                }
            }
        }
